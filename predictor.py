...
def make_prediction(new_image_path):
    ...
    try:
        ...
        for i in tqdm(range(0, height, chunk_size), desc="Predicting"):
            # ... (paste the chunking prediction loop from your notebook here) ...
            start_row = i
        ...
        subprocess.run(["python3", "reporter.py", "--map", prediction_path])
        
    except Exception as e:
        print(f"An error occurred in predictor: {e}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--input", required=True)
    args = parser.parse_args()
    make_prediction(args.input)